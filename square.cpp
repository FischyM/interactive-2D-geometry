...
string textureName = "goldy.ppm";  // test, goldy, brick
...
unsigned char* loadImage(int& img_w, int& img_h){
   ...
   for (int i = 0; i < img_h; i++){
      for (int j = 0; j < img_w; j++){
         ...
         img_data[i * img_w * 4 + j * 4] = red;
         img_data[i * img_w * 4 + j * 4 + 1] = green;
         img_data[i * img_w * 4 + j * 4 + 2] = blue;
         img_data[i * img_w * 4 + j * 4 + 3] = 255;
      }
   }

   // brighten the picture
   for (int i = 0; i < (4 * img_w * img_h); i++) {
       int temp = clamp(img_data[i] * 1.5, 0, 255);
       img_data[i] = temp;
   }
...
float dist(Point2D p1, Point2D p2) {
    return abs(vee(p1, p2).magnitude());
}

float dist(Line2D l, Point2D p) {
    return abs(vee(p, l.normalized()));
}
...
bool pointInBox(Point2D p, Point2D p1, Point2D p2, Point2D p3, Point2D p4) {
    bool orient1 = sign(vee(clicked_mouse, p1, p2)) == sign(vee(clicked_mouse, p2, p3));
    bool orient2 = sign(vee(clicked_mouse, p1, p2)) == sign(vee(clicked_mouse, p3, p4));
    bool orient3 = sign(vee(clicked_mouse, p1, p2)) == sign(vee(clicked_mouse, p4, p1));
    return orient1 && orient2 && orient3;
}

float pointLineDistance(Point2D p, Point2D p1, Point2D p2) {
    Point2D p_proj = dot(p, vee(p1, p2)) * vee(p1, p2);
    return dist(p, p_proj);
}

bool pointOnBoxLine(Point2D p, Point2D p1, Point2D p2, Point2D p3, Point2D p4) {
    if (pointLineDistance(p, p1, p2) < clickDistanceCheck || \
        pointLineDistance(p, p2, p3) < clickDistanceCheck || \
        pointLineDistance(p, p3, p4) < clickDistanceCheck || \
        pointLineDistance(p, p4, p1) < clickDistanceCheck){
        return true;
    } else {
        return false;
    }
...
// if p is near a corner, return true
bool pointOnBoxCorner(Point2D p, Point2D p1, Point2D p2, Point2D p3, Point2D p4) {
    return (dist(p, p1) < clickDistanceCheck) || (dist(p, p2) < clickDistanceCheck) || \
        (dist(p, p3) < clickDistanceCheck) || (dist(p, p4) < clickDistanceCheck);
}

// if p is near a corner, return that corner Point2D
Point2D pointBoxCorner(Point2D p, Point2D p1, Point2D p2, Point2D p3, Point2D p4) {
    if (dist(p, p1) < clickDistanceCheck) {
        return p1;
    } else if (dist(p, p2) < clickDistanceCheck) {
        return p2;
    } else if (dist(p, p3) < clickDistanceCheck) {
        return p3;
    } else if (dist(p, p4) < clickDistanceCheck) {
        return p4;
    } else {
        return p;
    }
...
void mouseClicked(float m_x, float m_y){   
   ...
   clicked_pos = rect_pos;  // Point2D
   clicked_angle = rect_angle;  // float
   clicked_size = rect_scale;  // float
   do_translate = false;
   do_rotate = false;
   do_scale = false;

   // if the click is on a corner
   if (pointOnBoxCorner(clicked_mouse, p1, p2, p3, p4)) {
       do_scale = true;
   } 
   // if the click is on an edge and not a corner
   else if (pointOnBoxLine(clicked_mouse, p1, p2, p3, p4)) {
        do_rotate = true;
    }
   // if the click is in the box
   else if (pointInBox(clicked_mouse, p1, p2, p3, p4)) {
       do_translate = true;
   }
}
...
void mouseDragged(float m_x, float m_y){
   ...
   if (do_scale){
       //Compute the new size, rect_scale, based on the mouse positions
       float scale1 = cur_mouse.x / clicked_mouse.x;
       float scale2 = cur_mouse.y / clicked_mouse.y;
       float scale;
       if (scale1 > scale2) { scale = scale1; }
       else { scale = scale2;  }
       rect_scale = clicked_size * scale;
   }
   
   if (do_rotate){
       //Compute the new angle, rect_angle, based on the mouse positions
       // find angle between the two lines
       Line2D l1 = vee(cur_mouse, rect_pos);
       Line2D l2 = vee(clicked_mouse, rect_pos);
       float angle = acos(dot(l1.normalized(), l2.normalized()));
       int sign_xy = sign(wedge(MultiVector(l1).normalized(), MultiVector(l2).normalized()).xy);
       if (sign_xy < 0) {
...
void r_keyPressed(){
   ...
   rect_pos = Point2D(0.0, 0.0);
   rect_scale = -1;
   rect_angle = 0;
   mouseDragged(0.0, 0.0);
}
